...
function onLoaded() {
  ...
  for (var i = 0; i < trArr.length; i++) {
    ...
    for (var j = 0; j < tdArr.length; j++) {
      ...
      tdArr[j].setAttribute('onClick', 'onCliked(event)');
    }
  }

  gidTiempo = setInterval(actualizarVisualizadorTiempo, 1000);
}
...
function eliminarClick(cartaSeleccionada) {
  cartaSeleccionada.removeAttribute('onClick');
}
...
function finalDelJuego() {
  var numFilas = document.getElementsByTagName('tr').length;
  var numColumnas = document.getElementsByTagName('tr')[0].getElementsByTagName('td').length;

  if (document.getElementsByClassName('cartaBocaArriba').length == numFilas * numColumnas) {
    return true;
  }
...
function actualizarVisualizadorTiempo() {
  var tiempo = numMinutos + ':' + numSegundos;
  numSegundos++;
